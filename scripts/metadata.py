...
DESCRIPTIONS = {
  ...
  "FiveFields": {
    "text": ("Message with five structured fields: string, int, double, Guid, and decimal."),
    "code": (
...
CAVEATS = {
  ...
  "Filtered": [
    ...
    {
      "logger": "ZLogger",
      "text": ("Built on MEL, so pays the same interface-dispatch and provider-iteration cost."),
    },
    {
      "logger": "Log4Net",
      "text": ("Walks a parent-child logger hierarchy to resolve the effective level."),
    },
    {
      "logger": "ClipMEL",
      "text": (
  ...
  "Console_FiveFields": [
    ...
    {
      "logger": "NLog",
      "text": ("Value-type arguments are boxed. Layout renderers write each property as a string."),
    },
    {
      "logger": "MEL",
      "text": (
        "Formats synchronously, then enqueues for background I/O. Value-type arguments are boxed."
      ),
    },
    {
      "logger": "MELSrcGen",
    ...
    },
    {
      "logger": "ClipMEL",
      "text": (
        "Same MEL template API as MEL, but formatting is handled by Clip's engine underneath."
      ),
    },
    {
      "logger": "ZeroLog",
  ...
  "Console_WithContext": [
    ...
    {
      "logger": "NLog",
      "text": ("ScopeContext pushes properties via AsyncLocal. Merged at layout render time."),
    },
    {
      "logger": "MEL",
      "text": (
    ...
    {
      "logger": "ClipMEL",
      "text": ("Uses MEL's BeginScope, then delegates to Clip's formatting engine."),
    },
  ],
  #
  ...
  "Console_WithException": [
    {
      "logger": "Clip",
      "text": ("Exception rendered synchronously into the same pooled byte buffer."),
    },
    {
      "logger": "Serilog",
      "text": ("Exception rendered synchronously, appended as a string to the output."),
    },
    {
      "logger": "NLog",
      "text": (
    ...
    {
      "logger": "ClipMEL",
      "text": ("Exception formatted by Clip's engine behind MEL's ILogger interface."),
    },
    {
      "logger": "ZeroLog",
      "text": (
  ...
  "Json_NoFields": [
    {
      "logger": "Clip",
      "text": (
        "Builds JSON as raw UTF-8 bytes into a pooled buffer. String values are escaped using SIMD."
      ),
    },
    {
      "logger": "Serilog",
    ...
    },
    {
      "logger": "MELSrcGen",
      "text": (
        "Source-generated — no template parsing. Same JsonConsoleFormatter pipeline as MEL."
      ),
    },
    {
      "logger": "ZLogger",
  ...
  "Json_FiveFields": [
    ...
    },
    {
      "logger": "Serilog",
      "text": (
        "Each argument is wrapped in a property object then serialized. Value types are boxed."
      ),
    },
    {
      "logger": "NLog",
      "text": ("Event properties are boxed and rendered through the layout engine as strings."),
    },
    {
      "logger": "MEL",
      "text": (
  ...
  "Json_WithContext": [
    ...
    },
    {
      "logger": "Serilog",
      "text": (
        "Context properties merged into the event object and serialized through the object model."
      ),
    },
    {
      "logger": "NLog",
      "text": ("Scope properties merged and rendered through the layout engine."),
    },
    {
      "logger": "MEL",
      "text": (
  ...
  "Json_WithException": [
    {
      "logger": "Clip",
      "text": (
        "Exception serialized as a structured JSON object synchronously into the pooled buffer."
      ),
    },
    {
      "logger": "Serilog",
    ...
    {
      "logger": "NLog",
      "text": ("Exception serialized as a JSON string attribute synchronously."),
    },
    {
      "logger": "MEL",
      "text": (
